...
${projectDescription}"
	...
	if [[ "${updateDescStatus}" != "200" ]]; then
		echo "Error updating project description. Status code: ${updateDescStatus}. Message: ${updateDescMessage}. Response: ${updateDescResponse}"
		exit 1
	fi
	echo " Done"
}
...
function importProject() {
	...
	projectNamespace=$(dirname "${projectPath}")
...
function migrateVariables() {
	...
	local variableUrlSourceGitlab
	local variableUrlTargetGitlab
	local response

	variableUrlSourceGitlab="${baseUrlSourceGitlabApi}/${type}/${entityEncoded}/variables?per_page=100"
	variableUrlTargetGitlab="${baseUrlTargetGitlabApi}/${type}/${entityEncodedTargetGitlab}/variables"

	response=$(curl ${CURL_PARAMS} -sS -w "\n%{http_code}" --header "${authHeaderSourceGitlab}" "${variableUrlSourceGitlab}")
	...
	if [[ "${httpResponse['status']}" == "200" ]]; then
		...
		for variable in "${variables[@]}"; do
			...
			varKey=$(jq -r '.key' <<<"${variable}")
			status=$(curl ${CURL_PARAMS} -sS -o /dev/null -w "%{http_code}" --header "${authHeaderTargetGitlab}" "${variableUrlTargetGitlab}/${varKey}")
...
function migrateBadges() {
	...
	for badge in "${badges[@]}"; do
		...
		badgeId=$(jq -r '.id' <<<"${badge}")
		status=$(curl ${CURL_PARAMS} -sS -o /dev/null -w "%{http_code}" --header "${authHeaderTargetGitlab}" "${badgesUrlTargetGitlab}/${badgeId}")
